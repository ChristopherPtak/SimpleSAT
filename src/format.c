...
Error read_problem(Solver *solver, FILE *stream)
{
    ...
    do {
        ...
        if (fgets(buffer, 255, stream) == NULL) {
            fprintf(stderr, "simplesat: Expected problem line\n");
    ...
    if (sscanf(buffer, "p cnf %d %d%n", &n_vars, &n_clauses, &pos) != 2) {
        fprintf(stderr, "simplesat: Invalid problem line\n");
        err = ERROR_INVALID_FORMAT;
        goto cleanup;
    }
    ...
    if (n_vars <= 0) {
        fprintf(stderr, "simplesat: Number of variables must be positive\n");
        err = ERROR_INVALID_FORMAT;
        goto cleanup;
    } else if (n_clauses <= 0) {
        fprintf(stderr, "simplesat: Number of clauses must be positive\n");
        err = ERROR_INVALID_FORMAT;
        goto cleanup;
    }
    ...
    for (i = 0; i < n_clauses; ++i) {
        ...
        for (;;) {
            ...
            if (fscanf(stream, " %d", &repr) != 1) {
                fprintf(stderr, "simplesat: Expected more clauses\n");
                err = ERROR_INVALID_FORMAT;
                goto cleanup_solver;
            }
    ...
    for (;;) {
        ...
        if (c == EOF) {
            ...
            fprintf(stderr, "simplesat: Expected end of input\n");
            err = ERROR_INVALID_FORMAT;
        }
    }
